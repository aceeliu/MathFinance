...
def get_yk(u, d, r, Sk, Xk, k, n, comb, i, cnt, lambda) : #return y
    ...
    if (k == n - 1) :
        ...
        if (k == 0) : 
            ...
            new_cnt = cnt + 2
        y1 = get_yk((Sk + lambda)/Sk, (Sk - lambda)/Sk, r, Sk + lambda, Xk_H, (k + 1), n, comb, i, new_cnt)
        y2 = get_yk((Sk + lambda)/Sk, (Sk - lambda)/Sk, r, Sk - lambda, Xk_T, (k + 1), n, comb, i, new_cnt)
        ret = (p * get_positive_part(y1) + q * get_positive_part(y2))
        return ret
        
    
def get_y0(u, d, r, s0, x0, k, n, comb, id): #implement formula of calculating y0
    y0 = (1+r) ** (-n) * get_yk((s0 + lambda)/s0, (s0 - lambda)/s0, r, s0, x0, 0, n, comb, id, 0)
    return y0
